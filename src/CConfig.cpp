...
bool CConfig::shouldIncludeFile(const std::string& filePath) const {
    ...
    if (!m_includePatterns.empty()) {
        for (const auto& pattern : m_includePatterns) {
            if (std::regex_match(filePath, pattern)) {
                return true; // 匹配包含模式，备份
            }
        }
        return false; // 有包含模式但未匹配，不备份
    }
...
bool CConfig::isValid() const {
    ...
    for (const auto& path : m_sourcePaths) {
        if (!std::filesystem::exists(path)) {
            std::cerr << "Error: Multiple source path does not exist: " << path << std::endl;
            return false;
        }
    }
    ...
    if (m_enableCompression && (m_compressionLevel < 1 || m_compressionLevel > 9)) {
        std::cerr << "Error: Compression level must be between 1 and 9 (current: " << m_compressionLevel << ")" << std::endl;
        return false;
    }
